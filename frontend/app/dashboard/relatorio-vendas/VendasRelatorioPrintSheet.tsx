...
export default function VendasRelatorioPrintSheet({
  ...
  const aVistaCount = entries.filter(e => e.pagamento === "À vista").length
  const prazoCount  = entries.length - aVistaCount
  ...
  return (
    <PrintShell wrapperClass="print-vendas-relatorio" pageMargin="12mm 14mm" onDone={onDone}>
      <div style={{ fontFamily: "'Arial', 'Helvetica', sans-serif", color: NAVY, fontSize: "9px" }}>
        ...
        {/* Entradas detalhadas — flui livre entre páginas */}
        <div className="print-avoid-break">
          <SectionTitle>Vendas no Período ({entries.length})</SectionTitle>
        </div>
        {entries.length === 0 ? (
            ...
            <tbody>
              {entries.map((e, i) => (
                <tr key={e.key} style={{ background: i % 2 === 0 ? "white" : NAVY_LIGHT, borderBottom: "1px solid #e0e4ec" }}>
                  ...
                  <td style={{ padding: "4px 8px", textAlign: "center", fontSize: "8px", color: e.pagamento.startsWith("Prazo") ? "#B45309" : "#059669" }}>{e.pagamento}</td>
