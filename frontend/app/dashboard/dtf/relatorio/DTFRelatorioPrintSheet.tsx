...
function pagamento(p: { dueDate: string | null; paidAt: string | null }): string {
  if (p.dueDate && !p.paidAt) return `Prazo · ${fmtDateOnlyBR(p.dueDate)}`
  if (p.dueDate && p.paidAt)  return "Prazo · Pago"
  return "À vista"
}
...
export default function DTFRelatorioPrintSheet({
  ...
  const aVistaCount = pedidos.filter(p => !p.dueDate).length
  const prazoCount  = pedidos.length - aVistaCount

  return (
    <PrintShell wrapperClass="print-dtf-relatorio" pageMargin="12mm 14mm" onDone={onDone}>
      <div style={{ fontFamily: "'Arial', 'Helvetica', sans-serif", color: NAVY, fontSize: "9px" }}>

        {/* Cabeçalho — repete em toda folha nova via thead, aqui só na 1ª página */}
        <div className="print-avoid-break">
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
            <img src="/smsemfundo.png" alt="SM Confecções" style={{ height: "46px", width: "auto", flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "18px", fontWeight: "900", letterSpacing: "-0.5px", lineHeight: 1 }}>SM CONFECÇÕES</div>
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
            {[
              ...
              { label: "Metro médio", value: metroMedio != null ? `${metroMedio.toFixed(2)} m` : "—" },
              { label: "Ticket médio", value: fmtR(ticketMedio) },
            ].map(s => (
              <div key={s.label} style={{ flex: 1, border: "1px solid #d8dde8", borderRadius: "4px", padding: "7px 9px" }}>
                <div style={{ fontSize: "6.5px", color: "#888", textTransform: "uppercase", letterSpacing: "0.6px" }}>{s.label}</div>
                <div style={{ fontSize: "12px", fontWeight: "800" }}>{s.value}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize: "8px", color: "#888", marginBottom: "14px" }}>
            {aVistaCount} à vista · {prazoCount} a prazo
          </div>
        </div>
        ...
        {/* Pedidos detalhados — tabela flui livre entre páginas, o cabeçalho
            (thead) se repete sozinho no topo de cada folha nova. */}
        <div className="print-avoid-break">
          <SectionTitle>Pedidos no Período ({pedidos.length})</SectionTitle>
        </div>
        {pedidos.length === 0 ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: NAVY, color: "white" }}>
                ...
                <th style={{ padding: "4px 8px", textAlign: "left",   fontSize: "8px", fontWeight: "700" }}>CLIENTE</th>
                <th style={{ padding: "4px 8px", textAlign: "right",  fontSize: "8px", fontWeight: "700" }}>METROS</th>
                <th style={{ padding: "4px 8px", textAlign: "center", fontSize: "8px", fontWeight: "700" }}>PAGAMENTO</th>
              ...
              {pedidos.map((p, i) => {
                ...
                return (
                  <tr key={p.id} style={{ background: i % 2 === 0 ? "white" : NAVY_LIGHT, borderBottom: "1px solid #e0e4ec" }}>
                    ...
                    <td style={{ padding: "4px 8px", textAlign: "center", fontSize: "8px", color: p.dueDate ? "#B45309" : "#059669" }}>{pagamento(p)}</td>
